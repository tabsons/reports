...
def story():
    if request.method == 'POST':
        ...
        if 'story standardization' in sheets:
            excel = pd.read_excel(file, sheet_name='story standardization')
            df['Story'] = excel['Suggested Headline']
        else:
            excel = pd.read_excel(file, sheet_name='expressreport')
            df['Story'] = excel['Story']
            ab = 'story'
